...
class Model:
    def __init__(self, num_params, model, lam, min_step):
        ...
        self.params = torch.tensor([10.0] * num_params)
        self.model = model
        self.lam = lam
...
def linear_model(x, w):
    return sigmoid((w[2]*x[1])+(w[1]*x[0])+w[0])

def gradient(fun):   
    def grad_core(x, delta=0.001):
        a = fun(x)
        b = fun(x+delta)
        return (b-a)/delta
    return grad_core
...
ran = torch.rand(10)
print(ran)
l = torch.linspace(-10,10,100)
dsigmoid = gradient(sigmoid)
ddsigmoid = gradient(dsigmoid)

plt.plot(l,sigmoid(l))
plt.plot(l,dsigmoid(l))
plt.plot(l,ddsigmoid(l))
plt.show()

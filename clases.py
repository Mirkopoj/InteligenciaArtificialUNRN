class Model:
    def __init__(self, num_params, model, lam, min_step):
        self.num_params = num_params
        self.params = [10.0] * num_params
        self.model = model
        self.lam = lam
        self.min_step = min_step

    def __error(self, y, x, params):
        return y - self.model(x,params)

    def __loss(self, dataset, params):
        ret = 0.0
        for dato in dataset:
            e = self.__error(dato[0], dato[1:],params)
            ret += e*e
        return ret

    def __gradient(self, dataset, xi, delta=0.0001):   
        a = self.__loss(dataset, self.params)
        xp = self.params
        xp[xi] += delta
        b = self.__loss(dataset, xp)
        return (b-a)/delta


    def train(self, dataset):
        avance = 1
        params2 = self.params
        while avance>self.min_step:
           avance = 0
           for i, w in enumerate(self.params):
               wprev = w
               params2[i] = w - self.lam*self.__gradient(dataset, i)
               avance += abs(params2[i] - wprev)
           self.params = params2
           avance /= self.num_params

    def solve(self, x):
        return self.model(x, self.params)

#######################################################################

def linear_model(x, w):
    return (w[2]*x[1])+(w[1]*x[0])+w[0]

datos = [
        [11, 4, 2],
        [7, 2, 2],
        [2, -1, 3],
        [1, 0, 0],
        [-2, -1, -1],
        ]

ia = Model(3, linear_model, 0.0001, 0.0000001)

print(ia.params)

ia.train(datos)

print(ia.params)

print("6*2+4+1=17")
iadice = ia.solve([6, 4])
print(iadice)
print("Error = ", abs(17-iadice))

